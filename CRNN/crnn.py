...
class CRNN(object):
                ...
                for batch_y, batch_dt, batch_x in self.__data_manager.train_batches:
                    ...
                    if i % 10 == 0:
                        for j in range(2):
                            ...
                            print("Count, Loss = ", count, loss_value)
    ...
    def test(self, save_test_img):
        if save_test_img and not(os.path.exists('./test_result/')):
        ...
        with self.__session.as_default():
            ...
            for batch_y, _, batch_x in self.__data_manager.test_batches:
                ...
                for i, y in enumerate(batch_y):
                    ...
                    if save_test_img:
                        img = cv2.transpose(batch_x[i]) 
                        # cv2.putText(img, str(ground_truth_to_word(decoded[i])), (50, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
                        # Uncomment this line to write tested images to directory with the name of the image as the predicted output by CRNN
                        cv2.imwrite("./test_result/"+ str(ground_truth_to_word(decoded[i]))+".jpg", img)
                    # print("i, Decoded=", i, decoded)
                    print("Predicted: ", ground_truth_to_word(decoded[i]))
        return None
